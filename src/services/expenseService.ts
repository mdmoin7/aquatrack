...
export async function getExpenseProvision(month: string): Promise<MonthlyExpenseProvision> {
  ...
  return provision ?? { id: month, billingMonth: month, collectionMonth: getNextMonth(month), paymentMonth: getNextMonth(month), updatedBy: '', updatedAt: '' }
}

export async function saveExpenseProvision(input: { billingMonth: string; collectionMonth: string; paymentMonth: string; residentNote?: string; updatedBy: string; snapshotGeneratedAt?: string; snapshotCutoffDate?: string; snapshotData?: ExpenseSnapshotData; surplusCarriedForward?: number; carryForwardMonth?: string }): Promise<MonthlyExpenseProvision> {
...
export async function deleteFundCollection(id: string): Promise<void> { await dataStore.deleteFundCollection(id) }
