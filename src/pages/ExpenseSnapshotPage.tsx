...
import { EXPENSE_CATEGORY_LABELS, type FundCollection, type MonthlyExpenseProvision, type SocietyExpense } from '@/types'
...
export function ExpenseSnapshotPage() {
  ...
  const shareImage = async () => {
    ...
    try {
      const canvas = createSnapshotCanvas({
        ...
        cutoff: cutoff ? new Date(`${cutoff}T00:00:00`).toLocaleDateString('en-IN', { day: 'numeric', month: 'long', year: 'numeric' }) : 'latest available record',
        collectedTotal,
        expenseTotal,
        carriedForward,
        surplus,
        categories: categories.map((item) => ({ label: EXPENSE_CATEGORY_LABELS[item.category], amount: item.amount })),
        residentNote: provision.residentNote,
        collectionMonth: formatMonthLabel(provision.collectionMonth),
        paymentMonth: formatMonthLabel(provision.paymentMonth),
      })
  ...
  const generateSnapshot = async () => {
    ...
    if (!latestCollectionDate) {
      setNotice('Record at least one funds-collected entry before generating a snapshot.')
      return
    }
    setSaving(true)
    try {
      ...
      await saveExpenseProvision({ ...provision, snapshotCutoffDate: cutoffDate, snapshotGeneratedAt: generatedAt, surplusCarriedForward, carryForwardMonth: getNextMonth(selectedMonth), updatedBy: user?.displayName || user?.email || 'Administrator' })
      setProvision({ ...provision, snapshotCutoffDate: cutoffDate, snapshotGeneratedAt: generatedAt, surplusCarriedForward, carryForwardMonth: getNextMonth(selectedMonth) })
  ...
  const addFundCollection = async () => {
    try {
      await saveFundCollection({ billingMonth: selectedMonth, ...collectionForm, amount: Number(collectionForm.amount), recordedBy: user?.displayName || user?.email || 'Administrator' })
      setCollectionForm({ collectedDate: new Date().toISOString().slice(0, 10), amount: '', source: 'Society maintenance collection', referenceNumber: '' }); await load()
...
function ImageMetric({ label, value, emphasize = false }: { label: string; value: string; emphasize?: boolean }) {
  return <div className="bg-white px-5 py-4"><p className="text-xs text-slate-500">{label}</p><p className={emphasize ? 'mt-1 text-base font-semibold text-emerald-600' : 'mt-1 text-base font-semibold text-slate-900'}>{value}</p></div>
}
...
function createSnapshotCanvas(data: {
  ...
  const metrics = [
    ...
    [data.surplus >= 0 ? 'Surplus carried forward' : 'Shortfall', formatCurrency(Math.abs(data.surplus)), data.surplus >= 0 ? '#059669' : '#e11d48'],
  ]
  metrics.forEach(([label, value, color], index) => {
    const col = index % 2; const row = Math.floor(index / 2); const x = pad + col * 480; const y = 316 + row * 126
    ctx.fillStyle = '#ffffff'; ctx.fillRect(x, y, 456, 104)
    ctx.fillStyle = muted; ctx.font = '400 22px system-ui, sans-serif'; ctx.fillText(label, x + 24, y + 38)
    ctx.fillStyle = color; ctx.font = '700 30px system-ui, sans-serif'; ctx.fillText(value, x + 24, y + 78)
  })
  let y = 624
  ctx.fillStyle = muted; ctx.font = '600 22px system-ui, sans-serif'; ctx.fillText('EXPENSE BREAKDOWN', pad, y)
  y += 46
  if (data.categories.length === 0) {
    ctx.fillStyle = muted; ctx.font = '400 26px system-ui, sans-serif'; ctx.fillText('No recorded expenses yet.', pad, y)
  } else {
    data.categories.forEach((item) => {
      ctx.strokeStyle = '#e2e8f0'; ctx.beginPath(); ctx.moveTo(pad, y + 16); ctx.lineTo(right, y + 16); ctx.stroke()
      ctx.fillStyle = text; ctx.font = '400 27px system-ui, sans-serif'; ctx.fillText(item.label, pad, y + 52)
      ctx.font = '700 27px system-ui, sans-serif'; const amount = formatCurrency(item.amount); ctx.fillText(amount, right - ctx.measureText(amount).width, y + 52)
      y += 64
    })
  }
  if (data.residentNote) {
    y += 34; ctx.strokeStyle = '#e2e8f0'; ctx.beginPath(); ctx.moveTo(pad, y); ctx.lineTo(right, y); ctx.stroke(); y += 42
    ctx.fillStyle = muted; ctx.font = '400 23px system-ui, sans-serif'
    const words = data.residentNote.split(' '); let line = ''
    for (const word of words) {
      if (ctx.measureText(`${line} ${word}`).width > width - pad * 2) { ctx.fillText(line, pad, y); y += 31; line = word } else line = `${line} ${word}`.trim()
    }
